...
1) Run `./gradlew :ytextractor:publishToMavenLocal`
2) Open `~/.m2/repository/com/kdroid/ytextractor/"}`
...
4) Add the secrets to `local.properties`:
   ...
   ```
5) Run `./gradlew :ytextractor:publishAllPublicationsToSonatypeRepository`
